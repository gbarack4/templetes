  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-4">
      ...
      <button
        ...
        onClick={onToggle}
        disabled={isBusy}
        className={`inline-flex h-9 min-w-[5.5rem] items-center justify-center rounded-lg px-3 text-xs font-semibold transition disabled:cursor-not-allowed ${
          connected
...
export function ConnectSignInLinksFlow() {
  ...
  const [connected, setConnected] = useState<ProviderState>({
    google: false,
    facebook: false,
  });
  const [busyProvider, setBusyProvider] = useState<SocialProvider | null>(null);

  function handleToggle(provider: SocialProvider) {
    ...
    <>
      ...
      <FlowPageContent>
        <p className="text-sm text-slate-500">
          Connect Google or Facebook to sign in faster next time.
        </p>
